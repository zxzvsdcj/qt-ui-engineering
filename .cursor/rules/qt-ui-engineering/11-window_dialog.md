...
推荐做法：
1. 用户必须完成当前决策才能继续同一工作流时使用`exec()`；保持对话框任务短小，不在显示期间启动不可控长任务。
2. 用户需要对照主窗口内容、并行查看多个窗口或持续使用工具面板时使用`show()`。
3. 非模态对话框必须由主窗口持有引用，或设置明确parent和删除策略；关闭时清理引用。
4. 长操作始终移入工作线程；模态不等于允许阻塞UI线程。

不推荐/禁止：
1. 禁止为了读取一个返回值把所有提示都改成`exec()`。
2. 禁止在局部变量中创建非模态对话框后立即`show()`而不保留生命周期。
3. 禁止在exec期间执行文件解析、网络请求或重计算。
